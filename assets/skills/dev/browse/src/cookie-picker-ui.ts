...
(function() {
  ...
  async function init() {
    try {
      ...
      if (browsers.length === 0) {
        $sourceDomains.textContent = '<div class="imported-empty">No Chromium browsers detected</div>';
        return;
      }
      ...
      browsers.forEach(b => {
        ...
        pill.className = 'pill';
        pill.textContent = '<span class="dot"></span>' + escHtml(b.name);
      ...
      $sourceDomains.textContent = '<div class="imported-empty">Failed to load</div>';
    }
  }
  ...
  async function selectBrowser(name) {
    ...
    $sourceDomains.textContent = '<div class="loading-row"><span class="spinner"></span> Loading...</div>';
  ...
  async function selectProfile(profileName) {
    ...
    $sourceDomains.textContent = '<div class="loading-row"><span class="spinner"></span> Loading domains...</div>';
    $sourceFooter.textContent = '';
    $search.value = '';
  ...
  async function loadDomains() {
    try {
      ...
      $sourceDomains.textContent = '<div class="imported-empty">Failed to load domains</div>';
    }
  }
  ...
  function renderSourceDomains() {
    ...
    if (filtered.length === 0) {
      $sourceDomains.textContent = '<div class="imported-empty">' +
    ...
    $sourceDomains.textContent = html;
  ...
  function renderImported() {
    ...
    if (entries.length === 0) {
      $importedDomains.textContent = '<div class="imported-empty">No cookies imported yet</div>';
    ...
    $importedDomains.textContent = html;
